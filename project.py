...
def showLogin():
    ...
    # return "The current login state is %s" %
    # login_session['state']
...
def gconnect():
    ...
    try:
        # Upgrade the authorization code into a
        # credentials object
    ...
    # Verify that the access token is used for the
    # intended user.
    ...
    # Verify that the access token is valid for
    # this app.
    ...
    # Store the access token in the session for
    # later use.
...
def catalogJSON():
    ...
    return jsonify(
        Categories=[r.serialize for r in result])
...
def catalogCategoryAdd():
    ...
    if request.method == 'POST':
        ...
        return redirect(
            url_for('showCatalogCategories'))
    else:
        return render_template(
            'addCatalogCategory.html')


@app.route(
    '/catalog/<int:category_id>/<string:category_item>/desc', methods=['GET'])
def showCatalogCategoryItemsItem(
...
@app.route('/catalog/<int:category_id>/<int:category_item>/edit',
           methods=['GET', 'POST'])
def showCatalogCategoryItemsItemEdit(
        category_id, category_item):
    if 'username' not in login_session:
        return redirect('/login')
    result = session.query(CategoryItem).filter_by(
...
@app.route('/catalog/<int:category_id>/<int:category_item>/delete',
           methods=['GET', 'POST'])
def showCatalogCategoryItemsItemDelete(
        category_id, category_item):
    if 'username' not in login_session:
        return redirect('/login')
    result = session.query(CategoryItem).filter_by(
